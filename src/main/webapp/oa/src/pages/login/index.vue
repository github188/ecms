    ...
    <form id="loginBox" @keyup.enter="verify">
      ...
      <div class="input-box">
        <div class="item-input">
          <input v-model.trim="username" autofocus placeholder="用户名" type="text">
          <i class="icon icon-user"></i>
        </div>
        <div class="item-input">
          <el-popover ref="capsLock" placement="right" :disabled="!capsLockStatus" trigger="focus" content="大写锁定已打开"></el-popover>
          <input v-popover:capsLock @keypress="capsLock($event)" v-model.trim="password" placeholder="密码" type="password">
...
export default {
  ...
  methods: {
    ...
    doLogin() {
      if (window.location.hostname == '192.168.9.84' && this.domain == "http://eap.ecaray.com") {
        alert("请注意当前连接的是正式环境！");
      }
      this.disable = true;
      let $this = this;
