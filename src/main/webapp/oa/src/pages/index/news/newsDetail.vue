              ...
              <ul class="file-list">
                <li v-for="item in fileList" :key="item.id">
                    <div v-if="isInstitution()" class="file-download" href="javascript:;">
                      <i class="el-icon-document" style="padding-right:10px;"></i>{{item.name}}
                    </div>
                    <a v-else title="附件下载" @click="downFile('/news/attachment/'+ item.id +'/download', item.name)" class="file-download" href="javascript:;">
                      <i class="el-icon-document" style="padding-right:10px;"></i>{{item.name}}
                    </a>

                  <router-link v-if="filePreview(item.name) =='pdf'" class="preview" :to="{path: '/pdf', query: {url: `/news/attachment/${item.id}/download`, name: item.fileName}}" target="_blank" title="在线预览"> 预览</router-link>
                  <a v-if="filePreview(item.name) =='img'" @click="previewUrl(item)" href="javasceipt:;" class="preview"> 预览</a>
            ...
            <ul class="comments-list">
              ...
              <li class="list-l" v-for="(item, index) in comments" :key="index" :id="'to'+item.id">
                <div class="c-area-l">
                  ...
                  <router-link :title="'查看'+ item.addPersonName +'个人主页'" target="_blank" :to="item.isDelete == 1? 'javascript:;':'/person/'+item.user.id" class="avatar-link">
                ...
                <div class="multi-reply" v-if="commentsChild[index].length > 0">
                  <li class="list-l" v-for="(child, childIndex) in commentsChild[index]" :key="child.id" :id="'to'+child.id">
                    <div class="c-area-l">
                      <router-link target="_blank" :title="'查看'+ child.addPersonName +'个人主页'" :to="child.isDelete == 1?'javascript:;':'/person/'+child.user.id" class="avatar-link">
...
export default {
  ...
  methods: {
    ...
    isInstitution(file){ //是否是制度版块，附件只能预览不能下载
      if(this.detail.typeName.split('#')[0] == '制度' || this.detail.newsType  == 3){
        return true
      }
    },
    filePreview(file) {
    ...
    previewUrl(item) {
        let $this = this
        let url = `/news/attachment/${item.id}/download`;
        this.imgModal = true
