        ...
        <div class="order-box order-detail-container">
          ...
          <template v-if="details.resultType == 3">
            <el-form-item label="出发地点：">
              <span> {{details.startAddress}} </span>
            </el-form-item>
            <el-form-item label="目的地点：">
              <span> {{details.endAddress}} </span>
            </el-form-item>
          </template>
      ...
      <template v-else>
        ...
        <div class="order-box order-detail-container">
          ...
          <el-form-item label="考勤状态：">
            <span class="red-color">{{dealExpName(details, details.expName)}}</span>
          </el-form-item>
        </div>
  ...
  methods: {
    dealExpName(details, name) {
      if (name) {
        ...
        if (
          details.expType == 3 &&
          this.details.resultType == 0 &&
          time == 1.0 &&
          (worktime && worktime >= 4)
        ) {
          //加班且不处理且加班为一天但实际打卡时间为0.5天
          return "加班0.5";
        }
        return status;
      }
        ...
        success(data, $this) {
          if (data.code == "success") {
            ...
            for (let i = 0; i < content.length; i++) {
              if (
                content[i].value == $this.details.resultObj &&
                $this.details.resultObj.type
              ) {
