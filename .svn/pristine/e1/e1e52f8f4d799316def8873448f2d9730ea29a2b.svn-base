...
headerObj.loginOut= function () {
    var url = headerObj.url + "/authority/account/logout",
        authorization = Utils.getValue("authorization");
    $.ajax({
        type: "GET",
        url: url,
...
headerObj.updateUserPwd = function () {
    ...
    var url = headerObj.url + "/authority/user/reset/password",
        authorization = Utils.getValue("authorization");
    var data={
        oldPass:userpwd,
        newPass:newpwd
...
headerObj.init= function () {
    ...
    headerObj.getEmailNum()
//    点击切换
//    $('.header-content .menu-list .menu-item').click(function (event) {
//        event.stopPropagation();
//        $(this).addClass('current').siblings('.menu-item').removeClass('current');
//    });
//    头部设置
