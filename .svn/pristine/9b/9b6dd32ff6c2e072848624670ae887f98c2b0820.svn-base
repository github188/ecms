...
var monthPicker = {
    ...
    create: function(id, conf) {
        ...
        if (that._conf.period && that._conf.start_month != '' && that._conf.end_month != '') {
            ...
            that._conf.autoCommit && that._conf.callback({ startDate: that._conf.start_month, endDate: that._conf.end_month });
    ...
    init: function() {
        ...
        $('#gri_preYear').click(function() {
            that.preYear(that.reset);
            $('#gri_month'+ that.assemble.month).addClass('current');
        });
    ...
    construct: function() {
        ...
        for (var p in months.data) {
            ...
            for (var i = 0; i < months.data[p].length; i++) {
                ...
                //判断是否超过今年
                var $ym = this.util.format({
                    year: this.assemble.year,
                    month: months.data[p][i]
                });
                if(parseInt($ym.year + $ym.month) < this._conf.startYearMonth){
    ...
    addCss: function(evt) {
        ...
        //target.setAttribute(that.util.getCla(), that._conf.selectCss);
        //提交
        if(target.className.indexOf(that._conf.disabledCss) ==  -1){
            that.removeCss();
            $(target).addClass(that._conf.selectCss);
            //赋值
